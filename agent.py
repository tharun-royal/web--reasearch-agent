...
def _sanitize_pdf_text(text: str) -> str:
    """
    Standard FPDF core fonts support only ISO-8859-1 (latin-1). 
    Replaces common smart quotes/dashes and strips out completely unencodable 
    characters to guarantee fpdf doesn't throw UnicodeEncodeError.
    """
    if not text:
        return ""
...
def build_pdf(query: str, report_text: str, sources: list) -> bytes:
    ...
    for raw_line in report_text.splitlines():
        ...
        # Clear Markdown bold tags from output text lines safely
...
.header{text-align:center; margin-bottom:28px; color:white;}
.header h1{font-size:32px; margin:0 0 6px 0; font-weight:700; letter-spacing:-0.5px;}
.header p{margin:0; color:rgba(255,255,255,0.7); font-size:15px;}
.card{background:var(--card); border-radius:20px; padding:28px; box-shadow:0 20px 60px rgba(0,0,0,0.35);}
.search-row{display:flex; gap:10px;}
.search-box{flex:1; position:relative;}
.search-box svg{position:absolute; left:14px; top:50%; transform:translateY(-50%); opacity:0.4;}
input{width:100%; padding:14px 14px 14px 42px; font-size:15px; border-radius:12px; border:1.5px solid #e5e7eb; outline:none;}
input:focus{border-color:var(--accent);}
button{padding:14px 22px; background:linear-gradient(135deg,var(--accent),#8b5cf6); color:white; border:none; border-radius:12px; cursor:pointer; font-size:15px; font-weight:600; white-space:nowrap; transition:transform .12s ease, box-shadow .12s ease;}
button:hover{transform:translateY(-1px); box-shadow:0 8px 20px rgba(99,102,241,0.35);}
button:disabled{opacity:0.6; cursor:not-allowed; transform:none; box-shadow:none;}
button.secondary{background:#f3f4f6; color:#374151;}
button.secondary:hover{box-shadow:0 8px 20px rgba(0,0,0,0.08);}
button.success{background:linear-gradient(135deg,#16a34a,#22c55e);}
.chips{display:flex; flex-wrap:wrap; gap:8px; margin-top:14px;}
.chip{padding:7px 13px; background:#f3f4f6; border-radius:999px; font-size:12.5px; color:#4b5563; cursor:pointer;}
.chip:hover{background:#eef2ff; color:var(--accent); border-color:#e0e7ff;}
.status{margin-top:18px; text-align:center; color:var(--text-dim); font-size:14px; min-height:20px;}
.spinner{width:18px; height:18px; border:2.5px solid #e5e7eb; border-top-color:var(--accent); border-radius:50%; display:inline-block; vertical-align:middle; margin-right:8px; animation:spin .7s linear infinite;}
@keyframes spin{to{transform:rotate(360deg);}}
.result{display:none; margin-top:24px; border-top:1px solid #eee; padding-top:22px;}
.result-toolbar{display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:6px; flex-wrap:wrap; gap:10px;}
.result-title-block strong{display:block; color:#111827; font-size:15.5px; margin-bottom:3px;}
.result-meta{color:#9ca3af; font-size:12.5px;}
.result-toolbar .actions{display:flex; gap:8px; flex-wrap:wrap;}
.result-toolbar button{padding:9px 16px; font-size:13px;}
.result-content{margin-top:16px;}
.result-content h3{color:var(--accent); font-size:16px; margin:18px 0 8px 0;}
.result-content p{color:#374151; line-height:1.65; font-size:14.5px; margin:0 0 10px 0;}
.result-content ul{margin:0 0 12px 0; padding-left:20px;}
.result-content li{color:#374151; line-height:1.6; font-size:14.5px; margin-bottom:4px;}
.sources{margin-top:8px; padding-top:18px; border-top:1px dashed #e5e7eb;}
.sources h3{color:var(--accent); font-size:16px; margin:0 0 10px 0;}
.source-card{display:flex; align-items:flex-start; gap:10px; padding:11px 12px; border-radius:12px; text-decoration:none; margin-bottom:8px;}
.source-card:hover{background:#f9fafb;}
.source-num{flex-shrink:0; width:22px; height:22px; border-radius:50%; background:#eef2ff; color:var(--accent); font-size:11.5px; font-weight:700; display:flex; align-items:center; justify-content:center;}
.source-text strong{display:block; color:#1f2937; font-size:13.5px; font-weight:600; margin-bottom:2px;}
.source-text span{color:#9ca3af; font-size:12px;}
.toast-stack{position:fixed; bottom:22px; right:22px; display:flex; flex-direction:column; gap:10px; z-index:999;}
.toast{background:#111827; color:white; padding:12px 18px; border-radius:10px; font-size:13.5px; box-shadow:0 10px 30px rgba(0,0,0,0.3); max-width:320px;}
...
function formatReport(text){
  ...
  for (let rawLine of lines){
    ...
    if(headingMatch && line.length < 100){
      if(inList){ html += "</ul>"; inList = false; }
...
Do not include a Sources section — sources will be handled separately.
"""
    try:
        ...
        if not report_text:
            return jsonify({"error": "Gemini returned no usable content."}), 502

        return jsonify({"report": report_text, "sources": sources})
    except Exception as e:
...
def download_pdf():
    ...
    if not FPDF_AVAILABLE:
        return jsonify({"error": "The package 'fpdf2' is missing on the server. Install it using: pip install fpdf2"}), 501

    try:
        pdf_bytes = build_pdf(query, report_text, sources)
    except Exception as e:
        return jsonify({"error": f"PDF structural failure: {str(e)}"}), 500
